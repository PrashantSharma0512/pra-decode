function pra_decode(message) {
    ...
    let bitLength = messageBytes.length * 8;

    messageBytes = Array.from(messageBytes);
    messageBytes.push(0x80);

    while ((messageBytes.length * 8 + 64) % 512 !== 0) {
        messageBytes.push(0x00);
    }

    for (let i = 7; i >= 0; i--) {
        messageBytes.push((bitLength >>> (i * 8)) & 0xff);
    }
    ...
    for (let i = 0; i < messageBytes.length; i += 64) {
        ...
        for (let j = 16; j < 64; j++) {
            let s0 = (w[j - 15] >>> 7) ^ (w[j - 15] >>> 18) ^ (w[j - 15] >>> 3);
            let s1 = (w[j - 2] >>> 17) ^ (w[j - 2] >>> 19) ^ (w[j - 2] >>> 10);
